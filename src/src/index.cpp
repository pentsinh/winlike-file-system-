...
void tree_make(struct My_filenode *node, int depth) // 目录树构建，外部调用时传入根目录
{
	...
	while (findnext(&ff) == 0)
	{
		if ((ff.ff_attrib & FA_DIREC) && strcmp(ff.ff_name, "..") != 0 && strcmp(ff.ff_name, ".") != 0) // 如果是文件夹
		{
	...
	while (findnext(&ff) == 0)
	{
		if ((ff.ff_attrib & FA_DIREC) && strcmp(ff.ff_name, "..") != 0 && strcmp(ff.ff_name, ".") != 0 && strcmp(ff.ff_name, p->name) != 0) // 如果是文件夹
		{
			...
			p->next->son_list_head = NULL;
			p->next->flag = 0;
			set_bit(&p->next->flag, 5, 0);	   // 标记非头节点
			if (depth == 0)					   // 第一级目录默认可见
				set_bit(&p->next->flag, 4, 1); // 展开
			p = p->next;
			if (depth < 2)
