...
void puthz(int x, int y, char *s, int flag, int part, int color)
{
	...
	part += flag; // 使得传入的part可以小于flag，更符合直觉
	...
	case 12:
	{
		...
		int y0 = y;
		int x0 = x;
		hzk_p = fopen("C:\\PROJECT\\src\\config\\HZK12", "rb"); // 使用绝对路径
		if (hzk_p == NULL)
		{
			settextjustify(LEFT_TEXT, TOP_TEXT);	 // 左部对齐，顶部对齐
			settextstyle(GOTHIC_FONT, HORIZ_DIR, 1); // 黑体笔划输出，水平输出，16*12点阵
	...
	case 16:
	{
		...
		int y0 = y;
		int x0 = x;
		hzk_p = fopen("C:\\PROJECT\\src\\config\\HZ16", "rb"); // 使用绝对路径
		if (hzk_p == NULL)
		{
			settextjustify(LEFT_TEXT, TOP_TEXT);	 // 左部对齐，顶部对齐
	...
	case 24:
	{
		...
		int y0 = y;
		int x0 = x;
		hzk_p = fopen("C:\\PROJECT\\src\\config\\Hzk24k", "rb");
		if (hzk_p == NULL)
		{
			settextjustify(LEFT_TEXT, TOP_TEXT);	 // 左部对齐，顶部对齐
	...
	case 32:
	{
		...
		int y0 = y;
		int x0 = x;
		hzk_p = fopen("C:\\PROJECT\\src\\config\\HZK32S", "rb");
		if (hzk_p == NULL)
		{
			settextjustify(LEFT_TEXT, TOP_TEXT);	 // 左部对齐，顶部对齐
			settextstyle(GOTHIC_FONT, HORIZ_DIR, 3); // 黑体笔划输出，水平输出，24*24点阵
	...
	case 48:
	{
		...
		int y0 = y;
		int x0 = x;
		hzk_p = fopen("C:\\PROJECT\\src\\config\\Hzk48k", "rb");
		if (hzk_p == NULL)
		{
			settextjustify(LEFT_TEXT, TOP_TEXT);	 // 左部对齐，顶部对齐
			settextstyle(GOTHIC_FONT, HORIZ_DIR, 3); // 黑体笔划输出，水平输出，24*24点阵
